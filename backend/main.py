...
# Use flash, but we will use the 'system_instruction' feature to give ELE its personality and rules
model = genai.GenerativeModel(
    ...
    Your JSON schema must be: {"reply": "Your conversational response to the user", "intent": "the intent", "action_required": true or false}"""
)
...
async def process_chat(user_input: UserInput):
    ...
    try:
        # 1. Ask the LLM (it is instructed to return JSON)
